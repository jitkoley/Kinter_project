...
class JSONEditorApp:
    def __init__(self, root):
        ...
        # Add logo using Pillow
        # Add logo using Pillow
        try:
            image_path = r"D:\project\poc\Image_logo\qc.png"  # Adjust path as needed
            img = Image.open(image_path)  # Open the image with Pillow
            
            # Check if image has transparency and convert it
            if img.mode in ("RGBA", "LA"):
                img = img.convert("RGBA")

            # Resize image to fit the window
            img = img.resize((150, 150), Image.Resampling.LANCZOS)
            self.logo = ImageTk.PhotoImage(img)  # Convert image to Tkinter-compatible format
            ...
            print(f"Error loading logo: {e}")  # Print error message if image fails to load
            # Fallback label if logo fails to load
            error_label = tk.Label(self.root, text="Logo could not be loaded.", font=("Arial", 12), fg="red", bg="#f0f0f0")
    ...
    def create_main_menu(self):
        ...
        tk.Label(self.root, text="Welcome to JSON Base \nSample_App Creator & Editor", font=("Arial", 20, "bold"), bg="#f0f0f0").pack(pady=10)

        tk.Button(self.root, text="Create New JSON File", font=("Arial", 12), width=30, height=2, command=self.create_new_file, bg="#007bff", fg="white", relief="flat", bd=0).pack(pady=10)
        tk.Button(self.root, text="Edit Existing JSON File", font=("Arial", 12), width=30, height=2, command=self.edit_existing_file, bg="#28a745", fg="white", relief="flat", bd=0).pack(pady=10)
